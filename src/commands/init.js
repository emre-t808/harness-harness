...
async function tryDelegate() {
  ...
  // Path is resolved relative to projectDir. Output is piped to stdout.
  ...
  try {
    ...
    const out = child.execFileSync('node', [abs], { input: stdin, env: process.env });
    process.stdout.write(out);
    return true;
  } catch (err) {
    process.stderr.write(\`[hh-assembler] delegate failed: \${err.message}\\n\`);
    return false; // Fall through to native assembler.
  }
}
