...
// The wrapper installed as .claude/hooks/hh-assembler.js. Kept as a template
// file (placeholder: __ASSEMBLER_SRC__) so tests can exercise the exact code
// init installs. Exported for those tests.
export function buildAssemblerWrapper(assemblerSrc) {
  const template = fs.readFileSync(path.join(TEMPLATES_DIR, 'hooks', 'assembler-hook.js'), 'utf8');
  return template.replace('__ASSEMBLER_SRC__', assemblerSrc);
}
...
export async function init(projectDir, flags) {
  ...
  if (!traceOnly) {
    ...
    fs.writeFileSync(path.join(paths.hooksDir, 'hh-assembler.js'), buildAssemblerWrapper(assemblerSrc), 'utf8');
    // Single-writer refactor: earlier versions installed a parallel
    // assembler-fallback.sh whose stale-marker check raced the assembler on
    // every prompt. The wrapper now emits its own fallback; remove any copy a
    // previous install left behind.
    try { fs.unlinkSync(path.join(paths.hooksDir, 'hh-assembler-fallback.sh')); } catch { /* not installed */ }
    console.log('  Assembler: installed as .claude/hooks/hh-assembler.js (built-in fallback)');
  }
...
function updateClaudeSettings(paths, traceOnly, replaceMode) {
  ...
  // -----------------------------------------------------------------------
  // UserPromptSubmit — Smart Assembler (fallback is built into the wrapper;
  // a second same-event hook would run in parallel and race it)
  // -----------------------------------------------------------------------
  if (!traceOnly) {
    if (replaceMode) {
