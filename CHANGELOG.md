...
- **Applicability + violation semantics in behavioral scoring** (`src/lib/rule-scoring.js`). A rule that declares `behavioral_signals` but whose triggers never fired now scores `null` with evidence `not-applicable` instead of `0.0`/`ignored` — downstream `parseSummary`'s numeric row regex intentionally skips `n/a` rows, so never-applicable sessions no longer feed false-negative zeros into Elo ratings and the promotion gate. New evidence states: `violated` (0.0 — an `expect.absent` trigger fired, or a `file_not_modified` guard was breached; trumps all positive evidence including explicit references) and `applicable-unmet` (0.0 — a trigger fired but no expectation was met; honest zero, distinguishable from "never applicable"). New signal schema fields: `trigger.tool_glob` (glob-match the tool name, e.g. `mcp__*{Google,google}*`), `trigger.input_regex` (regex on `input_summary`, e.g. `\bgws\b` on Bash commands), `expect.absent: true` (trigger firing IS the violation), `expect.present: true` (trigger firing IS the compliance), `expect.applicable_only: true` (trigger marks applicability only). Context: `scoreRuleCompliance` had zero callers since Phase 7/8 shipped; the Happiness install found 880 injections of one rule scoring 0.00 because only literal rule-ID citations counted. Follow-up (not in this change): port the `templates/hooks/session-summary.js` template to call `scoreRuleCompliance` so fresh installs get behavioral scoring without a hand-wired hook.
